...
export default function Popup({ post, setShowModal }) {
  ...
  return (
    <div
      className="fixed inset-0 z-40 rounded-md bg-gray-modalbg"
      onClick={() => setShowModal(false)}
    >
      <div className="flex flex-col z-50 sm:flex-row h-screen justify-center items-center mx-24">
        <div
          className="flex flex-col sm:flex-row sm:justify-center bg-white rounded-lg 2xl:h-880-px xl:h-740-px lg:h-520-px md:h-450-px sm:h-560-px popup"
          onClick={handleClick}
        >
          ...
          <div className="flex w-335-px sm:w-1/2 md:w-3/5 rounded-l-lg sm:h-full max-h-full bg-black-black">
            <Image src={post.imageSrc} caption={post.caption} contain />
          </div>
          <div className="flex-col items-stretch justify-between w-335-px sm:w-1/2 md:w-2/5 h-full min-h-full">
            ...
            <div
              className="overflow-y-auto hidden sm:block h-1/6 md:h-3/6 2xl:h-75% xl:h-70% lg:h-60%
