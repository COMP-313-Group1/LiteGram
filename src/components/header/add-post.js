...
export default function PostPopup({ user, setPostPopup }) {
  ...
  const addPostToFirebase = async () => {
    ...
    const newPost = await fireBase.firestore().collection('photos').add(post);

    await fireBase.firestore().collection('photos').doc(newPost.id).update({
      photoId: newPost.id,
            ...
            <div className="flex h-full w-full mt-10 items-center">
              ...
              <div className="w-2/5 h-full border border-gray-primary">
                <div className="flex items-center p-3">
                  ...
                  <img
                    className="w-32-px h-32-px rounded-full"
                ...
                <textarea
                  ...
                  className=" px-3 w-full focus:outline-none border-b border-gray-primary"
                  maxLength={2200}
                  rows={10}
                  value={post.caption}
                  onChange={(e) =>
                ...
                {/* <p className="text-center text-sm text-gray-primary">
                  More configuration - Future updates*
                </p> */}
              </div>
            </div>
          )}
